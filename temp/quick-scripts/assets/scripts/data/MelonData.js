(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/scripts/data/MelonData.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, 'e7e5faF9zxGhZc7hNJMWYHn', 'MelonData', __filename);
// scripts/data/MelonData.js
...
cc.Class({
    extends: CurData,
    ctor: function ctor() {},
    initNewTurn: function initNewTurn(turnIdx) {
    ...
    nextTurn: function nextTurn(chainToNext) {
    ...
    checkUpdateTree: function checkUpdateTree(row, col, oldMaterial, newMaterial) {
        ...
        //更新tree
        if (oldTypeMap.floor < newTypeMap.floor || oldTypeMap.block > newTypeMap.block) {
    ...
    deleteMinTree: function deleteMinTree(minTree, r, c) {
        var rebuildTreeArr = {};
        var deleteArr = {};
        ...
        for (var key in deleteArr) {
            ...
            var resArr = [];
            var resArrLen = this.traverseAround(deleteInfo.row, deleteInfo.col, resArr, function (row, col) {
    ...
    addMinTree: function addMinTree(minTree, r, c) {

        var rebuildTreeArr = [];
        var resArr = [];
        var resArrLen = this.traverseAround(r, c, resArr, function (row, col) {
            var aroundNode = cc.utils.getProperty(minTree, row, col);
            if (aroundNode) {
                return true;
            } else {
    ...
    updateMinTree_DelNode: function updateMinTree_DelNode(rootRow, rootCol, minTree, delr, delc) {
    ...
    updateMinTree_AddNode: function updateMinTree_AddNode(rootRow, rootCol, minTree, addr, addc) {
    ...
    updateSearchMapData: function updateSearchMapData(row, col, updateType) {
    ...
    optimizeSearchMapData: function optimizeSearchMapData() {
        var allMultiDir = {};
        for (var key in this.searchMapData) {
            var searchData = this.searchMapData[key];
            if (!searchData) {
    ...
    changeParent: function changeParent(minTree, info, sr, sl, fr, fl) {
    ...
    getDir: function getDir(row, col, fromRow, fromCol) {
        /*
                0
            5       1
    ...
    buildMinTree: function buildMinTree(rootRow, rootCol, minTree, row, col) {
        var buildGrid = function (sr, sl, fr, fl, len, forceBuild) {
            if (!this.isGridCanReach(sr, sl)) return;
            var fromLen = len + 1;
    ...
    initSearchMapData: function initSearchMapData() {
    ...
    getQuad: function getQuad(tr, tc, rr, rc) {
        if (rc <= tc) {
            if (rr >= tr) {
                return "quad2";
            } else {
    ...
    //from row
    //from col
    getDirPriority: function getDirPriority(tr, tc, rr, rc, fr, fc) {
    ...
    initGameData: function initGameData() {
        var _quad, _quad2, _quad3, _quad4;
        ...
        this.priorityConfig = {
            ...
            */
            ...
            quad1: (_quad = {}, _defineProperty(_quad, "0", 0), _defineProperty(_quad, "1", 5), _defineProperty(_quad, "2", 4), _defineProperty(_quad, "3", 0), _defineProperty(_quad, "4", 2), _defineProperty(_quad, "5", 1), _quad),
            ...
            */
            ...
            quad2: (_quad2 = {}, _defineProperty(_quad2, "0", 0), _defineProperty(_quad2, "1", 1), _defineProperty(_quad2, "2", 2), _defineProperty(_quad2, "3", 0), _defineProperty(_quad2, "4", 4), _defineProperty(_quad2, "5", 5), _quad2),
            ...
            */
            ...
            quad3: (_quad3 = {}, _defineProperty(_quad3, "0", 0), _defineProperty(_quad3, "1", 4), _defineProperty(_quad3, "2", 5), _defineProperty(_quad3, "3", 0), _defineProperty(_quad3, "4", 1), _defineProperty(_quad3, "5", 2), _quad3),
            ...
            */
            ...
            quad4: (_quad4 = {}, _defineProperty(_quad4, "0", 0), _defineProperty(_quad4, "1", 4), _defineProperty(_quad4, "2", 5), _defineProperty(_quad4, "3", 0), _defineProperty(_quad4, "4", 1), _defineProperty(_quad4, "5", 2), _quad4)
        };
    ...
    //////////////////////////////////////////////////////////////
    //游戏操作流程
    getSelectItemType: function getSelectItemType() {
        if (!this.selectItemObj) {
            return undefined;
        }
        return this.selectItemObj.type;
    },
    selectItem: function selectItem(itemType) {
        this.unSelectItem();
    ...
    consumeItem: function consumeItem() {
    ...
    addItem: function addItem(itemType, itemNum) {
    ...
    playerToHandle: function playerToHandle(row, col) {
    ...
    playerToHandleWithItem: function playerToHandleWithItem(row, col, material) {
    ...
    isGridContain: function isGridContain(material, typeMap) {
        if (!material) return;
        for (var matType in material) {
            if (!material[matType]) {
                continue;
    ...
    isGrid_CanBe_Destroy_Or_Add: function isGrid_CanBe_Destroy_Or_Add(material) {
        var typeMap = { floor: 0, role: 0, capper: 0, block: 0, target: 0, door: 0 };
        this.isGridContain(material, typeMap);
        if (typeMap.floor == 0 || typeMap.door > 0 || typeMap.role > 0 || typeMap.capper > 0 || typeMap.block > 0 || typeMap.target > 0) {
            return false;
        }
        return true;
    },
    destroyFloor: function destroyFloor(row, col, material) {
    ...
    playerToHandleWithoutItem: function playerToHandleWithoutItem(row, col, material) {
        if (!this.isGrid_CanBe_Destroy_Or_Add(material)) {
    ...
    isGridCanReach: function isGridCanReach(row, col) {
        if (row < 0 || col < 0) {
            return false;
        }
        var material = cc.utils.getProperty(this.topMapData, row, col, "material");
        ...
        this.isGridContain(material, typeMap);
        if (typeMap.floor == 0 && typeMap.door == 0 || typeMap.block > 0) {
            return false;
        }
        return true;
    },
    traverseAround: function traverseAround(row, col, resArr, judgeFunc, caller) {
        row = parseInt(row);
        col = parseInt(col);
    ...
    roleToGrid: function roleToGrid(roleInfo, toRow, toCol) {
    ...
    roleToHandle: function roleToHandle() {
        var hasPath = false;
...
cc._RF.pop();
        ...
        //# sourceMappingURL=MelonData.js.map
